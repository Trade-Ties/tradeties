...
const WHEN_OPTIONS: { mode: Exclude<WhenMode, "custom">; label: string; sublabel: string }[] = [
  ...
  { mode: "flexible", label: "I'm flexible", sublabel: "See the next 7 days" },
];
...
export function HeroSearch() {
  ...
  return (
    <>
      {/*
        Three levels, deliberately: the page layout is a flex column
        (`(marketplace)/layout.tsx`), and a flex item that carries its own
        `mx-auto`/`max-w` directly gets centered instead of stretched — that
        was the actual bug, not a caching issue. So `sticky` goes on its own
        unconstrained wrapper (a plain flex item, same shape as the hero
        `<section>`, which is why *that* was never off), the 1180px/px-6
        content frame is a normal (non-flex-item) div inside it, and the
        bar itself — unchanged, `ml-0`, left-aligned, not centered — is
        inside that.
      */}
      ...
      <div>
        <div className="mx-auto max-w-[1180px] px-6">
          <div className="animate-tt-rise mt-[22px] mb-10 flex flex-wrap items-center gap-2 [animation-delay:280ms]">
