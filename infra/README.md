...
`TestcontainersConfiguration` are both pinned to `postgres:18-alpine`; keep the
deployed version at or above the tested one.
...
role that is permitted to — the Compose superuser is, but on a managed database
that is usually a one-off grant by an administrator, not something the
application user has by default. Get it wrong and the very first migration run
fails.
